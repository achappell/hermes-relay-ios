...
- `_bmad-output/implementation-artifacts/next-wave-standard-hermes-migration/stories/validation-0-I-4-apple-migrate-ios-macos-client.md` -- record fake-bridge results plus live iOS/macOS text, voice, reconnect, route/rollback, interruption, timing-absence, and content-safe evidence using the existing manual plan -- makes this surface’s gate independently reviewable.
