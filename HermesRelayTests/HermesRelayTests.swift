...
final class HermesRelayIOSTests: XCTestCase {
    ...
    func testUnconfiguredDoorwayMakesConfigurationThePrimaryAction() {
        let state = ConversationDoorwayState(
            connectionState: .disconnected,
            profileName: nil
        )

        XCTAssertEqual(state, .unconfigured)
        XCTAssertEqual(state.statusLabel, "Not configured")
        XCTAssertEqual(state.recoveryMessage, "Configure a Hermes relay profile to begin.")
        XCTAssertEqual(state.primaryActionTitle, "Configure relay")
        XCTAssertFalse(state.isVoiceAvailable)
    }

    func testUnavailableDoorwayMakesRetryThePrimaryAction() {
        let state = ConversationDoorwayState(
            connectionState: .failed("The relay did not acknowledge the session."),
            profileName: "Home"
        )

        XCTAssertEqual(state, .unavailable("The relay did not acknowledge the session."))
        XCTAssertEqual(state.statusLabel, "Unavailable")
        XCTAssertEqual(state.primaryActionTitle, "Retry")
        XCTAssertFalse(state.isVoiceAvailable)
    }

    func testConnectedDoorwayKeepsVoiceAvailable() {
        let state = ConversationDoorwayState(
        ...
        XCTAssertNil(state.recoveryMessage)
        XCTAssertNil(state.primaryActionTitle)
        XCTAssertTrue(state.isVoiceAvailable)
    }
